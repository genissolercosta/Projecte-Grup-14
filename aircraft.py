...
class Aircraft:#Aquesta classe, serveix perquè, cada vegada que llegim el fitxer, crearà un aerport per cada línia, amb
...
def PlotArrivals (aircrafts):#Grafiquem la freqüència d'arribada per cada hora
    ...
    while i<len(aircrafts):
        hora_avio=int(aircrafts[i].land_time[:2])#L'hora la tenim en format "hh:mm" (ex: "03:14"). Tallem els dos
        # primers caràcters [:2] i els convertim a número enter, a través de int.
        vols_cada_hora[hora_avio]=vols_cada_hora[hora_avio]+1#Sumem 1 a la posició corresponent d'aquella hora.
        i=i+1
    ...
    try:
        ...
        while i < len(aircrafts):
            if (aircrafts[i].aircraft_id!="" and
                ...
                #Aquesta part de comprovar que existeixin els 4 camps no caldria, ja hem descartat les línies errònies
                # a la funció LoadAirports
                F.write("- - - -\n")
            i=i+1
        F.close()
...
def LongDistanceArrivals(aircrafts, airports):#Classifica els vols que tenen una distància superior als 2000km respecte
# LEBL
    resultat=[]
    if len(aircrafts)==0 or len(airports)==0:
        return resultat
