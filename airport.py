...
import simplekml    #Permet generar arxius en el format que utilitza Google Earth, per tal de poder-los anar ficant al mapa.
...
def RemoveAirport(airports,code):    #Busca l'aeroport a la llista. En cas que el trobi, l'esborra, amb la funció "delete".
    ...
    if encontrado:
        ...
        airports=airports[:-1]
        """
        del airports[i] #Per evitar-nos una variable local i haver de redefinir to tel codi, utilizem delete, tot i que ho podríem fer com està comentat a dalt
        return 0
...
def MapAirports(airports):  #Revisa la llista d'aeroports, i crea un punt al mapa per a cadascun d'ells. En cas que sigui de l'espai Schengen, el pinta de color blau i, sinó, de color vermell.
    kml=simplekml.Kml()

    for airport in airports:
        if airport.Schengen:
            punto=kml.newpoint(name=airport.ICAO,coords=[(airport.longitud,airport.latitud)])
            punto.style.iconstyle.color=simplekml.Color.blue
        else:
            punto=kml.newpoint(name=airport.ICAO,coords=[(airport.longitud,airport.latitud)])
            punto.style.iconstyle.color=simplekml.Color.red

    kml.save("Airports.kml")    #Guarda l'arxiu.
